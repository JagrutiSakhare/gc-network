...
environments_regex="^(dev|tuc|tud|prod|shared)$"
...
tf_plan() {
  ...
  if [ -d "$path" ]; then
    cd "$path" || exit
    terraform plan -input=false -var="terraform_service_account=${_TF_SA_EMAIL}" -out="${tmp_plan}/${tf_component}-${tf_env}.tfplan" || exit 21
    cd "$base_dir" || exit
  else
