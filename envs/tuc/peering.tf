...
data "google_compute_network" "non-internet" {
  ...
  depends_on = [module.non-internet-vpc]
}
...
module "peering-from-non-in" {
 ...
 depends_on = [module.non-internet-vpc, module.landing_zone]
}
...
module "peering-from-dmz" {
 ...
 depends_on = [module.dmz-vpc, module.landing_zone]
}
